...
class ShipVelocity(LinearVelocity): 
        
    def __init__(self, 
                 m : float = global_config.DEFAULT_PARAMS['ShipVelocity']['m'], 
                 d : float = global_config.DEFAULT_PARAMS['ShipVelocity']['d'], 
                 start_state : np.array =None, 
                 desired_state : np.array =np.array([1.0, 0.6, 0.0]), 
                 Ts : float = global_config.DEFAULT_PARAMS['ShipVelocity']['Ts'], 
                 render_mode=None,
                 max_episode_len : int = global_config.DEFAULT_PARAMS['ShipVelocity']['max_episode_len'], 
                 seed = None, 
                 noise_std : float = 0.0,
                 update_frequency : int = 20, 
                 action_allowed_coef : float = 1): 
        ...
        """
        super().__init__(m,d,start_state, desired_state, Ts, render_mode, max_episode_len, seed=seed, noise_std=noise_std, update_frequency=update_frequency)    
        self.action_space = gym.spaces.Box(low = action_allowed_coef * np.array([-5,-5, -100]), high = action_allowed_coef * np.array([5,5, 100]), dtype = float)
        # augmented_state_space = [gym.spaces.Box(low = -3, high = 3, dtype = float) , gym.spaces.Box(low = 0, high = 5, dtype = float), gym.spaces.Box(low = 0, high = 5, dtype = float)]  + augmented_actions
        self.observation_space = gym.spaces.Box(low = np.array([-2, -2, -0.25]), high = np.array([10,2, 0.25]), dtype = float)
        self.observation_space.seed(seed = seed)
        self.m = np.array(m)
        self.d = np.array(d)
        self.vel_weights = np.array([[1e3, 0, 0], [0, 1e3, 0], [0,0,5e4]])


    def reward_fun(self, action):
        vel_reward = np.sqrt(np.sum(self.vel_weights * np.square(self.state - self.desired_state)))
        manually_calc = np.sqrt(np.sum([np.diag(self.vel_weights)[i] * (self.state[i] - self.desired_state[i])**2 for i in range(len(self.state))]))
        # print(f"Vel reward {vel_reward} and computed manually {manually_calc}")
        torque_penalty = 0.01 * np.sum((action/self.action_space.high)**2)
        goal_reward = 1 if np.sqrt(np.sum(np.square(self.state - self.desired_state))) < 0.1 else 0

        return - vel_reward - torque_penalty + goal_reward
    
    def get_new_state(self, action):
        new_state = self.state + np.matmul(np.linalg.inv(self.m), (action - np.matmul(self.d, self.state)))  * self.Ts
        return new_state
    
    # def reset(self, seed=None, options=None):
    #     ret = super().reset(seed, options)
    #     self.desired_state[2] = self.state[2]
    #     return ret
