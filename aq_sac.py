...
SelfAQSAC = TypeVar("SelfAQSAC", bound="AQSAC")
...
class AQSAC(SAC):
    ...
    def __init__(self, policy: str | MlpPolicy, 
        ...
        if num_delays is None:
            num_delays = global_config.ENV_NUM_DELAYS[env.unwrapped.__class__.__name__]
        self.num_delays = num_delays
        if env is not None:
            self.env_model = deepcopy(env.env.env)
            self.max_delay = env.known_delay
        else : 
            self.max_delay = global_config.ENV_MAX_DELAY[env.unwrapped.__class__.__name__]
        self.avg = avg
        print(f"Max delay {self.max_delay}, num delays {self.num_delays}")
