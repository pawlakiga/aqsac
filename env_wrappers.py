...
class VariableActionDelay(gym.Wrapper) : 
    ...
    def __init__(self, env: gym.Env,
        ...
        if delay_type != 'external' and delay_type != 'wear' and delay_type != 'velocity-dependent' :
    ...
    def update_current_delay(self): 
        ...
        elif self.delay_type == 'velocity-dependent' :
            if self.unwrapped.timestep % self.update_freq == 0 :
                self.current_delay = min(int(self.gain * self.state[0]), self.max_delay)
                # print(f"Current delay is {self.current_delay}")
        else :
            return
...
class RandomiseSetpoint(gym.Wrapper):
    def __init__(self, env: gym.Env, min_setpoint : float = None, max_setpoint : float = None, use_regions : bool = False):
        super().__init__(env)
        if min_setpoint is None : 
            min_setpoint = self.env.observation_space.low + 0.05 * (self.env.observation_space.high - self.env.observation_space.low)
        if max_setpoint is None :
            max_setpoint = self.env.observation_space.high - 0.05 * (self.env.observation_space.high - self.env.observation_space.low)
    ...
    def reset(self, seed = None, options = None) :
        ...
        self.desired_state = np.random.uniform(self.min_setpoint, self.max_setpoint, size = self.env.observation_space.shape)
...
def init_wrappers(core_env : gym.Env, 
    ...
    if randomise_setpoint:
        core_env = RandomiseSetpoint(core_env, use_regions=core_env.__class__.__name__ != 'SphericalTank')
